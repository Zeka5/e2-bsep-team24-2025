...
public class CSRServiceImpl implements CSRService {
    ...
    public CSRResponse createCSR(CreateCSRRequest request, User requester) {
        try {
            if (request.getRequestedType() == com.example.bsep_backend.pki.domain.CertificateType.ROOT_CA) {
                throw new IllegalArgumentException("Cannot create CSR for ROOT_CA type. Root certificates can only be created by administrators.");
            }

            Certificate selectedCA = certificateRepository.findBySerialNumber(request.getSelectedCaSerialNumber())
    ...
    public CSRResponse reviewCSR(Long csrId, ReviewCSRRequest request, User reviewer) {
        ...
        if (request.getApproved()) {
            ...
            try {
                Certificate issuedCertificate = createCertificateFromCSR(csr, reviewer);
    ...
    private CSRResponse mapToResponse(CertificateSigningRequest csr) {
        return CSRResponse.builder()
                ...
                .requestedType(csr.getRequestedType())
                ...
                .selectedCaCommonName(csr.getSelectedCA() != null ? csr.getSelectedCA().getCommonName() : null)
                .issuedCertificateSerialNumber(csr.getIssuedCertificate() != null ?
                        csr.getIssuedCertificate().getSerialNumber() : null)
                .build();
    }

    private Certificate createCertificateFromCSR(CertificateSigningRequest csr, User reviewer) throws Exception {
        ...
        certRequest.setCertificateType(csr.getRequestedType());
        certRequest.setValidityDays(csr.getRequestedValidityDays());
        certRequest.setParentCaSerialNumber(csr.getSelectedCA().getSerialNumber());
